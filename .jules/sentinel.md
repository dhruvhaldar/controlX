...
## 2026-05-30 - Prevent Framework Exception Leakage in Frequency Response Calculations
**Vulnerability:** The `calculate_singular_values`, `system_gain`, and `calculate_hinf_norm` functions allowed raw framework-level exceptions (such as `ValueError` from `python-control` zero denominator checks or `TypeError`s) to leak when internal `sys.frequency_response` or `ct.evalfr` evaluations failed.
**Learning:** In mathematical analysis tools, directly returning the result of terminal library calls (like `ct.evalfr` or `frequency_response`) without exception handling exposes underlying library internals and generic stack traces to the user upon failure. This violates the principle of failing securely.
**Prevention:** Always wrap terminal numerical calculations or third-party API calls like `ct.evalfr` or `frequency_response` in a `try...except Exception` block, and explicitly raise a domain-specific `ValueError` (e.g., "Failed to evaluate system frequency response") to mask internal implementation details.
## 2024-05-20 - Prevent Silent Data Truncation and Loss via Strict Array Casting
**Vulnerability:** Core functions `relative_gain_array` and `small_gain_theorem_check` forced `dtype=float` when converting inputs to NumPy arrays, causing complex numbers (like frequency responses) to have their imaginary parts silently discarded with only a warning, leading to mathematically incorrect results.
**Learning:** Forcing a specific numeric type (like `float`) on scientific mathematical inputs without checking their intrinsic type (e.g., `np.iscomplexobj`) can cause silent data loss (truncation of imaginary components). This can corrupt downstream safety margins and control system design calculations.
**Prevention:** Always validate numeric inputs safely using `np.asarray` and dynamically check for complex objects (`np.iscomplexobj`) before explicitly casting to either `complex` or `float`.
## 2025-10-30 - Prevent Silent Data Truncation and Loss via Strict Array Casting
**Vulnerability:** Core functions `_validate_matrix`, and input array validation in `mpc.py` and `synthesis.py` forced `dtype=float` when converting inputs to NumPy arrays, causing complex numbers to have their imaginary parts silently discarded with only a warning, leading to mathematically incorrect results.
**Learning:** Forcing a specific numeric type (like `float`) on scientific mathematical inputs without checking their intrinsic type (e.g., `np.iscomplexobj`) can cause silent data loss (truncation of imaginary components). This can corrupt downstream safety margins and control system design calculations.
**Prevention:** Always validate numeric inputs safely using `np.asarray` and dynamically check for complex objects (`np.iscomplexobj`) before explicitly casting to either `complex` or `float`.
## 2024-05-26 - Prevent Exception Leakage in Synthesis Eigenvalue Computations
...
## 2026-06-26 - Prevent Unhandled Exception Leakage from Fallback Solvers
...
## 2025-02-27 - Properly validate complex numbers to prevent exception leakage
**Vulnerability:** Weak input validation swallowed and leaked incorrect exceptions for complex inequality bounds and optimization variables.
**Learning:** `try...except ValueError` blocks that wrap `np.iscomplexobj` checks with `raise ValueError(...)` cause the custom validation messages to be caught immediately by the surrounding handler, preventing the targeted error messages from functioning correctly and leaking downstream optimization solver exceptions instead.
**Prevention:** Move input type checking (like `np.iscomplexobj` and its associated `raise ValueError`) strictly outside of broad `try...except (TypeError, ValueError)` typecasting blocks.
