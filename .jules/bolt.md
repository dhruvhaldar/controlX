...
## 2024-05-24 - Speedup system_gain via direct matrix solve
**Learning:** In python-control, evaluating individual frequencies using `ct.evalfr` for StateSpace systems is slow due to library overhead and validation. We can achieve a 5-9x speedup by directly solving the linear system `sys.C @ np.linalg.solve(s * np.eye(sys.nstates) - sys.A, sys.B) + sys.D`. This requires handling scalar extraction for SISO systems (`res[0, 0]`) and using `np.nan` for `LinAlgError` to maintain API compatibility.
**Action:** When evaluating state space systems at specific frequencies, directly implement the mathematical formulation using `numpy.linalg.solve` rather than relying on `ct.evalfr`, while maintaining proper error handling for singular matrices.
